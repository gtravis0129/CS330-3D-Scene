...
void ViewManager::Mouse_Position_Callback(GLFWwindow* window, double xMousePos, double yMousePos)
{
	...
	float yOffset = gLastY - (float)yMousePos; // reversed: y ranges bottom to top
	gLastX = (float)xMousePos;
	gLastY = (float)yMousePos;
...
void ViewManager::Mouse_Scroll_Callback(GLFWwindow* window, double xOffset, double yOffset)
{
	...
	if (gCameraSpeed < 1.0f)  gCameraSpeed = 1.0f;
...
void ViewManager::ProcessKeyboardEvents()
{
	...
	// precompute right vector for horizontal strafing
...
void ViewManager::PrepareSceneView()
{
	...
	{
		// orthographic view (2D-style)
		float orthoSize = 10.0f;                 // adjust to frame heater
